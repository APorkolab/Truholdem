...
import com.truholdem.service.HandRanking;
...
public class HandEvaluator {
    ...
    private HandRanking evaluate5CardHand(List<Card> hand) {
        ...
        if (isStraight && isFlush) {
            ...
            if (highCard == Value.ACE) return new HandRanking(HandRanking.HandType.ROYAL_FLUSH, List.of(highCard), List.of());
            return new HandRanking(HandRanking.HandType.STRAIGHT_FLUSH, List.of(highCard), List.of());
        }
        ...
        if (!fourOfAKindRanks.isEmpty()) {
            return new HandRanking(HandRanking.HandType.FOUR_OF_A_KIND, fourOfAKindRanks, getKickers(counts, fourOfAKindRanks));
        }
        ...
        if (!threeOfAKindRanks.isEmpty() && !pairRanks.isEmpty()) {
            ...
            return new HandRanking(HandRanking.HandType.FULL_HOUSE, fullHouseRanks, List.of());
        }

        if (isFlush) {
            return new HandRanking(HandRanking.HandType.FLUSH, List.of(), hand.stream().map(Card::getValue).toList());
        }

        if (isStraight) {
            Value highCard = isWheel ? Value.FIVE : hand.get(0).getValue();
            return new HandRanking(HandRanking.HandType.STRAIGHT, List.of(highCard), List.of());
        }

        if (!threeOfAKindRanks.isEmpty()) {
            return new HandRanking(HandRanking.HandType.THREE_OF_A_KIND, threeOfAKindRanks, getKickers(counts, threeOfAKindRanks));
        }

        if (pairRanks.size() == 2) {
            return new HandRanking(HandRanking.HandType.TWO_PAIR, pairRanks, getKickers(counts, pairRanks));
        }

        if (pairRanks.size() == 1) {
            return new HandRanking(HandRanking.HandType.ONE_PAIR, pairRanks, getKickers(counts, pairRanks));
        }

        return new HandRanking(HandRanking.HandType.HIGH_CARD, List.of(), hand.stream().map(Card::getValue).toList());
    }
